...
- Criação do service `storage-favoritos` com funções de manipulação do Storage ( armazenamento físico no aparelho)
- Carregamento dos dados com auxílio do useEffect
- Exbição em FlatList
- Uso de funções intermediárias em favoritos.tsx para acionar as funções do storage para salvar, apagar, apagarTudo.
...
- Adiciona os dados do filme como string para o parâmetros da rota de detalhes
- Em detalhes, captura o filme como string e converte para o objeto
- Exibe os dados do filme nos componentes
- Formata a data e ajusta lógica para exibir "Não disponivel"
  quando não tiver data e sinopse
